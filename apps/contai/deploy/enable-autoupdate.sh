...
# De 5 em 5 minutos (temporizador systemd), se o ramo remoto tiver commits novos, o servidor faz git pull e reconstroi a app.
...
wanted = {"/usr/local/bin/contai-autoupdate", "/usr/local/bin/contai-update", "/etc/systemd/system/contai-autoupdate.service", "/etc/systemd/system/contai-autoupdate.timer"}
...
EOF_PY
rm -f /etc/cron.d/contai-autoupdate
systemctl daemon-reload && systemctl enable --now contai-autoupdate.timer
/usr/local/bin/contai-autoupdate || true
echo "actualizacao automatica activa (temporizador systemd de 5 em 5 minutos); registo em /var/log/contai/autoupdate.log"

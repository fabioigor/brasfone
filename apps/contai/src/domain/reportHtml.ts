...
<div class="foot">${d.sector ? `Referências sectoriais: ${esc(d.sector.source)}. ${esc(d.sector.disclaimer)}<br>` : ""}Gerado por Cont.ai by Lumarcont em ${esc(d.generatedAt.slice(0, 16).replace("T", " "))}. Documento informativo; não substitui as demonstrações financeiras oficiais.</div>
</div></body></html>`;
}

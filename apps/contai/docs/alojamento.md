...
>
> **Servidor de produção (criado em 2026-09-13 pela API):** `contai`, id 165711943, tipo cx23 (2 vCPU, 4 GB, 40 GB; o cx33 não tinha capacidade em nenhum datacenter nesse dia, e o tipo pode ser aumentado depois sem reinstalar), Helsínquia (hel1, Finlândia, UE), IPv4 `89.167.73.120`, firewall `contai-web` (22/80/443). Provisionado com `deploy/hetzner-create.sh`; utilizador `contai` (sudo), Docker + Caddy, backups diários em `/var/backups/contai`. Domínio ainda por definir (acesso HTTP por IP); ao definir o DNS, correr `echo app.contai.pt > /etc/contai-domain && contai-update` no servidor para activar o TLS.

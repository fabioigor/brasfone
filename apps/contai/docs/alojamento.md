...
>
> **Servidor de produção (criado em 2026-09-13 pela API):** `contai`, id 165724442, tipo cpx32 (4 vCPU AMD, 8 GB, 160 GB NVMe; escolhido por ser o tipo x86 com melhor relação desempenho/preço para OCR local e builds Docker; pode subir de tipo sem reinstalar, mas não descer, porque o disco não encolhe), Falkenstein (fsn1, Alemanha, UE; com Nuremberga, o datacenter Hetzner mais próximo de Portugal, cerca de 40 a 50 ms de latência), IPv4 `188.34.165.178`, firewall `contai-web` (22/80/443). Provisionado com `deploy/hetzner-create.sh`; utilizador `contai` (sudo), Docker + Caddy, backups diários em `/var/backups/contai`, actualização automática a partir do ramo. Domínio ainda por definir (acesso HTTP por IP); ao definir o DNS, correr `echo app.contai.pt > /etc/contai-domain && contai-update` no servidor para activar o TLS.

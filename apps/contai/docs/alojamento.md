...
>
> **Servidor de produção (criado em 2026-09-13 pela API):** `contai`, id 165726154, tipo cpx32 (4 vCPU AMD, 8 GB, 160 GB NVMe; escolhido por ser o tipo x86 com melhor relação desempenho/preço para OCR local e builds Docker; pode subir de tipo sem reinstalar, mas não descer, porque o disco não encolhe), Falkenstein (fsn1, Alemanha, UE; com Nuremberga, o datacenter Hetzner mais próximo de Portugal, cerca de 40 a 50 ms de latência), IPv4 `159.69.51.149`, firewall `contai-web` (22/80/443). Provisionado com `deploy/hetzner-create.sh`; utilizador `contai` (sudo), Docker + Caddy, backups diários em `/var/backups/contai`, actualização automática a partir do ramo. Domínio ainda por definir (acesso HTTP por IP); ao definir o DNS, correr `echo app.contai.pt > /etc/contai-domain && contai-update` no servidor para activar o TLS.
...
O servidor corre `contai-autoupdate` de 5 em 5 minutos (temporizador systemd `contai-autoupdate.timer`, com PATH completo e registo em journal): faz `git fetch` e, se o ramo remoto tiver commits novos, corre `contai-update` (pull + rebuild da imagem + reinício sem perder o volume `/data`). Assim, **um push para o ramo chega a produção em menos de 10 minutos**, sem chave SSH e sem token da API do Hetzner. Registo em `/var/log/contai/autoupdate.log`, também visível na app em Integrações > Sistema (versão, último commit aplicado, últimas linhas do registo), para diagnosticar sem SSH. Os scripts `contai-update` e `contai-autoupdate` do servidor são wrappers finos: a lógica está em `apps/contai/deploy/update.sh` e `deploy/autoupdate.sh`, no repositório, e actualiza-se também por git. `GET /health` devolve a versão do `package.json` para confirmar que build está a correr. Servidores criados antes desta versão activam-no uma vez, como root (consola web do Hetzner): `contai-update && bash /home/contai/brasfone/apps/contai/deploy/enable-autoupdate.sh`. Quando a PR for integrada em `main`, mudar o ramo seguido com `cd /home/contai/brasfone && runuser -u contai -- git checkout main`.

### Domínio e TLS sem SSH: cartão "Domínio e TLS" em Integrações

O gabinete escreve o domínio (ex.: `app.lumarcont.pt`) em Configuração > Integrações > Domínio e TLS. A app guarda-o na pasta partilhada `/var/lib/contai/config` (montada em `/config` no contentor); o temporizador de actualização nota a mudança em até 5 minutos, reaplica o `docker compose` com `CONTAI_DOMAIN` e o Caddy pede o certificado Let's Encrypt logo que o registo A do domínio aponte para o IP do servidor. O cartão mostra o IP a usar no DNS, para onde o domínio resolve, se já foi aplicado e se o TLS responde. O ficheiro `/etc/contai-domain` continua a funcionar como alternativa por consola (`contai-update`). O DNS de lumarcont.pt está no Wix (ns6/ns7.wixdns.net): o registo cria-se em Wix > Domínios > Gerir registos DNS.

### Segredos das integrações: página "Integrações" na app

As chaves da Anthropic, do email (webhook e IMAP), do WhatsApp e do CentralGest configuram-se em **Configuração > Integrações** (só gabinete), cada grupo com um botão **Testar com os valores acima** (Anthropic: lista de modelos; IMAP: ligação e contagem da pasta; WhatsApp: consulta do número na Graph API) antes de guardar. Ficam cifradas na base de dados (AES-256-GCM, chave derivada de `CONTAI_SECRET_KEY` ou, na sua ausência, de `JWT_SECRET`), sobrepõem-se ao `.env` e nunca voltam ao browser em claro (só os últimos 4 caracteres). Guardar reinicia a app em produção (poucos segundos) para todos os componentes recarregarem. O `.env` do servidor só precisa, por isso, de `JWT_SECRET`, `CONTAI_ADMIN_*` e, opcionalmente, `CONTAI_SECRET_KEY`.

...
- **API HTTP:** 68 rotas (lista gerada em `docs/api.md`). **MCP:** 25 ferramentas `contai_*` e `centralgest_*` (`src/mcp/server.ts`, stdio).
...
- CentralGest: cliente contra contrato assumido (`docs/centralgest-api.md`), simulador, teste de ligação, mapa de códigos, despacho idempotente; CSV Primavera como alternativa.
